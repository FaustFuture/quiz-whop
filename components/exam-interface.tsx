...
export function ExamInterface({ questions, moduleTitle, companyId, moduleId, userId, userName, moduleType, isUnlocked }: ExamInterfaceProps) {
	...
	const renderImages = (imgs: string[]) => {
		...
		if (imgs.length === 2) {
			if (layout === 'vertical') {
				return (
					<div className={`relative w-full mb-4 rounded-lg overflow-hidden bg-muted border border-border p-1 sm:p-2 ${getImageHeightClass()}`}>
			...
			return (
				<div className={`relative w-full mb-4 rounded-lg overflow-hidden bg-muted border border-border p-1 sm:p-2 ${getImageHeightClass()}`}>
					<div className="flex gap-2 sm:gap-4 h-full overflow-x-auto scrollbar-hide">
						{imgs.map((u, i) => (
							<div key={i} className="relative w-[280px] sm:w-80 h-full group flex-shrink-0">
								<img src={u} alt={`Question image ${i + 1}`} className="w-full h-full object-cover rounded" />
								<div className="absolute top-1 left-1 px-1.5 py-0.5 bg-black/70 text-white text-xs font-bold rounded">
									{i + 1}
								</div>
			...
			<div className="max-w-2xl mx-auto px-2 sm:px-4">
				<Card className="border-gray-200/10">
					<CardHeader className="p-4 sm:p-6">
						<CardTitle className="text-xl sm:text-2xl lg:text-3xl text-center">{moduleType === 'module' ? 'Quiz Results' : 'Exam Results'}</CardTitle>
					</CardHeader>
					<CardContent className="space-y-4 sm:space-y-6 p-4 sm:p-6 pt-0">
						<div className="text-center">
							...
							{!isLoadingStats && retakeStats && retakeStats.totalAttempts > 0 && (
								<div className="mt-4 sm:mt-6 p-3 sm:p-4 bg-muted rounded-lg">
									<h4 className="font-semibold mb-2 sm:mb-3 text-sm sm:text-base">Your Quiz History</h4>
									<div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 text-xs sm:text-sm">
										<div className="text-center">
											<div className="text-xl sm:text-2xl font-bold text-primary">{retakeStats.totalAttempts}</div>
											<div className="text-muted-foreground">Total Attempts</div>
										</div>
										<div className="text-center">
											<div className="text-xl sm:text-2xl font-bold text-green-600">{retakeStats.bestScore}%</div>
											<div className="text-muted-foreground">Best Score</div>
										</div>
										<div className="text-center">
											<div className="text-xl sm:text-2xl font-bold text-blue-600">{retakeStats.latestScore}%</div>
											<div className="text-muted-foreground">Latest Score</div>
										</div>
										<div className="text-center">
											<div className="text-xl sm:text-2xl font-bold text-purple-600">{retakeStats.averageScore}%</div>
						...
						<div className="space-y-3 sm:space-y-4">
							<h3 className="font-semibold text-base sm:text-lg">Question Breakdown</h3>
								...
								return (
									<div key={question.id} className="flex items-start sm:items-center gap-2 sm:gap-3 p-2 sm:p-3 rounded-lg border">
										...
										<div className="flex-1 min-w-0">
											...
											<p className="text-xs sm:text-sm text-muted-foreground break-words">{question.question}</p>
										</div>
							...
							>
								<div className="flex items-start justify-between gap-2 sm:gap-3">
									<div className="flex-1 min-w-0">
										...
										<p className="font-medium text-sm sm:text-base">{alternative.content}</p>
					...
					{questions.map((_, index) => {
						...
						return (
							<button
								...
								className={`h-2 rounded-full transition-all touch-manipulation ${isCurrent ? "w-8 bg-primary" : hasAttempt ? "w-2 bg-green-500" : "w-2 bg-secondary"
									}`}
				...
				{questions.map((_, index) => {
					...
					return (
						<button
							key={index}
							onClick={() => setCurrentQuestionIndex(index)}
							className={`h-2 rounded-full transition-all touch-manipulation ${isCurrent
									? "w-8 bg-primary"
									: isAnswered
										? "w-2 bg-green-500"
										: "w-2 bg-secondary"
					...
					<div className="relative w-full h-full flex items-center justify-center">
						...
						<div className="w-full h-full flex items-center justify-center px-2">
							{/^https?:\/\/(www\.)?(youtube\.com|youtu\.be)\//.test(fullscreenVideo) ? (
								...
								<video controls className="max-w-full max-h-full object-contain w-full">
