...
export async function MemberModulesView({
  companyId,
  userId,
}: MemberModulesViewProps) {
  const modules = await getModules(companyId);
  const grants = await getUserRetakeGrants(userId);
  const retakeAllowed = new Set<string>(
    grants.success ? (grants.data as string[]) : []
  );
...
async function ModuleExamCard({
  ...
  // Debug logging
  console.log(
    `Module: ${module.title}, User: ${userId}, Has Result: ${hasResult}`
  );
  if (result) {
        ...
        {hasResult ? (
          <div className="space-y-6 flex flex-col flex-1">
            ...
            <div className="mt-auto grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              >
                <Button
                  className="w-full gap-2 bg-muted border-border text-muted-foreground hover:text-foreground hover:bg-accent hover:border-emerald-500"
                  variant="outline"
                >
